...
local soundDistance = 3 -- Distance of sounds lock / unlock (default: 10m)
...
AddEventHandler("ls:lock", function(lockStatus, vehicle)
	...
	if lockStatus == 1 or lockStatus == 0 then -- Si le véhicule est déverrouillé (on le verrouille):
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Veículo trancado.", 0.080)
		-- ## Notifications
	...
	elseif lockStatus == 2 then -- Si le véhicule est verrouillé
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Veículo destrancado.", 0.080)
		-- ## Notifications
