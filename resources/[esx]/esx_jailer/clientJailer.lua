	...
	if DoesEntityExist(pP) then
		...
		Citizen.CreateThread(function()
			...
			while jT > 0 and not eJE do
				...
				if jT % 30 == 0 then
					TriggerEvent('chatMessage', 'SYSTEM', { 0, 0, 0 }, jT .." segundos antes de ser liberado.")
				end
				Citizen.Wait(500)
				...
				if D > 2 then -- distance#######################################################################################
					...
					if D > 4 then
						...
						TriggerEvent('chatMessage', 'JUDGE', { 0, 0, 0 }, "Seu tempo de prisão foi estendido.")
					end
				end
				jT = jT - 0.5
			end
			TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." foi libertado da prisão.")
			...
			while jT > 0 and not eJE do
				...
				if D > 2 then
					...
					if D > 4 then
						...
						TriggerEvent('chatMessage', 'JUDGE', { 0, 0, 0 }, "Seu tempo na prisão foi estendido.")
					end
				end
				jT = jT - 0.5
			end
